...
function verifyCnpj(cnpj: string): boolean {
  ...
  if (calculateDigit(cnpj.substring(0, 12), 9) !== parseInt(cnpj.charAt(12)))
    return false;
  if (calculateDigit(cnpj.substring(0, 13), 9) !== parseInt(cnpj.charAt(13)))
    return false;
...
export default function verifyCpfCnpj(input: string): {
  isValid: boolean;
  error: string;
  cpfCnpjFormat: string;
  cpfCnpj: string;
} {
